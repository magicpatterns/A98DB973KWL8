...
export function Calendar({
  ...
  return (
    ...
    >
      <div
        className={cn("localize" + "-calendar", {
          ["localize" + "-calendar-variant-" + "variant"]:
      ...
      >
        {calendars.map((calendar, index) => (
          <div
            key={calendar.month}
            className={cn("localize" + "-calendar-container")}
          >
            <div
              key={calendar.month}
              className={cn("localize" + "-calendar-header")}
            >
              {index === 0 && (
                <CalendarSelector direction="left" renderProps={renderProps} />
              )}
              {numOfMonths > 1 && index === calendars.length - 1 && <div />}
              <div className={cn("localize" + "-calendar-header-text")}>
            ...
            <div className={cn("localize" + "-calendar-body")}>
...
function CalendarTable({ calendar }: { calendar: Calendar }) {
  return (
    <>
      <div className={cn("localize" + "-calendar-table")}>
        <div className={cn("localize" + "-calendar-weekday-row")}>
          {WEEKDAY_NAMES.map((weekday) => (
            <div
              key={calendar.month + calendar.year + weekday}
              className={cn("localize" + "-calendar-cell")}
            >
              {weekday}
            </div>
          ))}
...
function CalendarSelector({
  ...
  return (
    <button
      className={cn("localize" + "-calendar-header-icon-button")}
      {...(direction === "left"
        ? getBackProps({ calendars })
        : getForwardProps({ calendars }))}
    >
      <FontAwesomeIcon
        className={cn("localize" + "-calendar-header-icon")}
...
function CalendarCell({
  ...
  if (!dateObj) {
    return <div className={cn("localize" + "-calendar-cell")} />;
  }
  ...
  return (
    <div
      className={cn("localize" + "-calendar-cell", {
        ["localize" + "-calendar-cell-disabled"]:
          date.getMonth() !== calendar.month || !selectable,
        ["localize" + "-calendar-cell-selected"]: selected,
        ["localize" + "-calendar-cell-in-range"]: isCellInRange,
        ["localize" + "-calendar-cell-selected-left"]:
          isSelectedLeft && selectionType === "range",
        ["localize" + "-calendar-cell-selected-right"]:
